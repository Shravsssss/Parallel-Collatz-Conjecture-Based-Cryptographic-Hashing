...
// Compute a basic Collatz-based hash by XOR-ing the number with its step count.
unsigned long long collatz_hash(unsigned long long n) {
    return n ^ collatz_steps(n);
}

// Apply SHA-256 mixing for an 8-byte input (the Collatz-based hash).
...
int main(int argc, char *argv[]) {
    ...
    FILE *out = NULL;
    if (rank == 0) {
        out = fopen("64_64_hybrid_results.txt", "w");
    ...
    for (unsigned long long global_end = 2000000; global_end <= 15000000; global_end += 1000000) {
        ...
        // Allocate
        unsigned long long *local_collatz = malloc(local_n * sizeof(*local_collatz));
        unsigned char (*local_digests)[SHA256_DIGEST_LENGTH] = 
            malloc(local_n * SHA256_DIGEST_LENGTH);
        if (!local_collatz || !local_digests) {
            fprintf(stderr, "Rank %d: Allocation failure\n", rank);
            MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
        }

        // Stage 1: Collatz (OpenMP)
        ...
        // Reduce timings and gather digests
